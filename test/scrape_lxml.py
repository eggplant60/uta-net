...
def main():
    ...
    tree = lxml.html.parse(urlopen(URL_ARTIST_DEFAULT))
    html = tree.getroot()
...
def save_song(db_path, song_data):
    """
    引数 song_data で与えられた dict を SQLite データベースに保存
    db_path: データベースのパス
    return:  なし
    """
    conn = sqlite3.connect(db_path)
    c = conn.cursor()
    #c.execute('DROP TABLE IF EXISTS books') # すでにテーブルが存在する場合は削除
    try:
        c.execute('''
        CREATE TABLE songs(title text, artist text, lyricist text, comporser text, lyric text)
        ''')
    except:
        pass
    c.execute('INSERT INTO songs VALUES (:title, :artist, :lyricist, :comporser, :lyric)', song_data)
    conn.commit()
    conn.close()
